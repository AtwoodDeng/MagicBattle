...
namespace Obi{
	...
	public class ObiParticleBakerEditor : Editor
	{
		...
		public override void OnInspectorGUI() {
			...
			if (!baker.Baking){
				if (GUILayout.Button(new GUIContent("Bake",EditorGUIUtility.Load("Obi/RecButton.psd") as Texture2D))){
					baker.Baking = true;
				}
			}else{
				if (GUILayout.Button(new GUIContent("Stop Bake",EditorGUIUtility.Load("Obi/StopRecButton.psd") as Texture2D))){
					baker.Baking = false;
				}
			}
			
			if (!baker.Playing){
				if (GUILayout.Button(new GUIContent("Play",EditorGUIUtility.Load("Obi/PlayButton.psd") as Texture2D))){
					baker.Playing = true;
				}
			}else{
				if (GUILayout.Button(new GUIContent("Stop",EditorGUIUtility.Load("Obi/StopButton.psd") as Texture2D))){
					baker.Playing = false;
				}
			}

			if (!baker.Paused){
				if (GUILayout.Button(new GUIContent("Pause",EditorGUIUtility.Load("Obi/PauseButton.psd") as Texture2D))){
					baker.Paused = true;
				}
			}else{
				if (GUILayout.Button(new GUIContent("Resume",EditorGUIUtility.Load("Obi/StepButton.psd") as Texture2D))){
					baker.Paused = false;
				}
			}
