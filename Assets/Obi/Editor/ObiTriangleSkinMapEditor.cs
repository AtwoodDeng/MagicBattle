	...
	public class ObiTriangleSkinMapEditor : Editor
	{
		...
		public void GetMaterials(){
			if (paintMaterial == null)
				paintMaterial = ObiEditorUtils.GetRequiredEditorResource("Obi/PropertyGradientMaterial.mat") as Material;
		...
		void UpdateEditMode(){
			...
			paintMode = GUILayout.Toggle(paintMode,new GUIContent("Paint skin master/slave",EditorGUIUtility.Load("Obi/PaintButton.psd") as Texture2D),"LargeButton");

			// skin channel selector:
			if (paintMode){
				GUILayout.BeginHorizontal();

				if (GUILayout.Toggle(brushType == PaintBrushType.Add,new GUIContent("Add",EditorGUIUtility.Load("Obi/AddIcon.psd") as Texture2D),GUI.skin.FindStyle("ButtonLeft")))
					brushType = PaintBrushType.Add;

				if (GUILayout.Toggle(brushType == PaintBrushType.Remove,new GUIContent("Remove",EditorGUIUtility.Load("Obi/RemoveIcon.psd") as Texture2D),GUI.skin.FindStyle("ButtonMid")))
					brushType = PaintBrushType.Remove;

				if (GUILayout.Button(new GUIContent("Fill",EditorGUIUtility.Load("Obi/FillButton.psd") as Texture2D),GUI.skin.FindStyle("ButtonMid"))){
				...
				if (GUILayout.Button(new GUIContent("Clear",EditorGUIUtility.Load("Obi/ClearButton2.psd") as Texture2D),GUI.skin.FindStyle("ButtonRight"))){
					if (sourceObject != null && Selection.activeGameObject == sourceObject){
						for (int i = 0; i < skinMap.masterFlags.Length; ++i)
							skinMap.masterFlags[i] &= ~(uint)(1 << selectedSkinChannel);
					}
		...
		public void OnSceneGUI(SceneView sceneView){
			...
			// Update paintbrush:
			ObiClothParticleHandles.ParticleBrush(wsPositions,ObiParticleActorEditor.FaceCulling.Back,facingCamera,brushRadius,
												 	()=>{
