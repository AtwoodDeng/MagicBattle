...
public abstract class ObiClothBase : ObiActor
{
	...
	[HideInInspector][SerializeField] protected Oni.NormalsUpdate normalsUpdate = Oni.NormalsUpdate.Recalculate;
	[HideInInspector][SerializeField] protected bool updateTangents = false;

	[HideInInspector][SerializeField] protected ObiMeshTopology sharedTopology;	/**< Reference mesh topology used to create a particle based physical representation of this actor.*/
	...
	public ObiAerodynamicConstraints AerodynamicConstraints{
		get{return constraints[Oni.ConstraintType.Aerodynamics] as ObiAerodynamicConstraints;}
	}
    ...
    {
        EventHandler handler = OnDeformableMeshSetup;
        if (handler != null)
        {
            handler(this, null);
        }
    ...
    {
        EventHandler handler = OnDeformableMeshTeardown;
        if (handler != null)
        {
            handler(this, null);
        }
	...
	protected void InitializeWithRegularMesh(){
		...
		/*for(int i = 0; i < topology.heVertices.Length; i++){
			...
			int[] visualVertices = topology.GetVisualVertices(i);
			for (int j = 0; j < visualVertices.Length; ++j){
				meshVertices[visualVertices[j]] = positions[i];
			}
	...
	public void Optimize(){
		...
		for (int i = 0; i < topology.heVertices.Length; ++i){
			...
			if (!active[i]){
				...
				foreach (KeyValuePair<Oni.ConstraintType,ObiBatchedConstraints> constraint in constraints){

					// for each constraint batch (usually only one)
					foreach (ObiConstraintBatch batch in constraint.Value.GetBatches()){
