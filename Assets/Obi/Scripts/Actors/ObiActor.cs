...
public abstract class ObiActor : MonoBehaviour, IObiSolverClient
{
	...
	[HideInInspector] public int[] deformableTriangles;		/**< Indices of deformable triangles (3 per triangle)*/
	[NonSerialized] protected int trianglesOffset;			/**< Offset of deformable trtiangles in curent solver*/
	...
	public virtual bool RemoveFromSolver(object info){
		...
		if (solver != null && InSolver){
			...
			for (int i = actorID+1; i < solver.actors.Count; i++){
	...
	public virtual void PushDataToSolver(ParticleData data = ParticleData.NONE){
		...
		for (int i = 0; i < particleIndices.Length; i++){
			...
			if ((data & ParticleData.VORTICITIES) != 0 && i < vorticities.Length)
				Oni.SetParticleVorticities(solver.OniSolver,new Vector4[]{l2sTransform.MultiplyVector(vorticities[i])},1,k);
